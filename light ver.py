...
alert_threshold = config.get('alert_threshold', 1800)
...
# Funkcja wysyłająca logi na e-mail
...
# Uruchomienie monitoringu i wysyłanie logów cyklicznie
def monitor_and_send_logs():
    while True:
        ...
        send_email(
            subject="Logi monitoringu aplikacji",
            body="Załączam logi z monitorowania aplikacji.",
            to_email=email_settings.get('to_email', ''),
            from_email=from_email,
            from_password=from_password,
            files=["app_usage_log.csv"]
        )

        # Przerwa przed kolejnym cyklem monitorowania (np. 1 godzina)
        time.sleep(duration_seconds)

# Uruchomienie monitoringu i wysyłanie logów
